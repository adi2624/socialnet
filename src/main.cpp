...
void print(TSN::request D) 
{
    ...
    for (unsigned int i = 0; i < D.user_requests.length(); i++) 
    {
        ...
        {
            ss << "Fulfiller UUID: " << D.user_requests[i].fulfiller_uuid << std::endl;
        }
...
void print(TSN::user_information D) 
{
    ...
    ss << "User UUID: " << D.uuid << std::endl << D.first_name << " " << D.last_name;
    print_queue.push(ss.str());
}
...
void print(TSN::response D) 
{
    ...
    ss << "User UUID: "<< D.uuid <<  std::endl << "Posts: " << std::endl << "ID: " << D.post_id 
                << std::endl << "Post: " << D.post_body << std::endl
                << "Date: " << D.date_of_creation ;
    print_queue.push(ss.str());
}
